...
void del(int x,int l,int r,int tar)
{
	if(l==r&&r==tar)
	{
		t[x]--;lazytag[x]--;//似乎减了也没用 
		return;
	}
	if(l>tar||r<tar) return;
	pushdown(x,l,r);
	int mid=l+(r-l>>1);
	if(mid>=tar) del(x<<1,l,mid,tar);
	if(mid<tar) del(x<<1|1,mid+1,r,tar);
	t[x]=t[x<<1]+t[x<<1|1];
}
void change(int x,int l,int r,int ll,int rr)
{
	...
	if(l>=ll&&r<=rr)
	{
		lazytag[x]++;t[x]+=(r-l+1);							//pushdown(x,l,r);
		return;
	}
	pushdown(x,l,r);
	int mid=l+(r-l>>1);
...
void query2(int x,int l,int r,int tar)
{
	if(l==r&&r==tar)
	{
		printf("%d\n",t[x]);return;
	...
	int mid=l+(r-l>>1);
	pushdown(x,l,r);
	if(mid>=tar) query2(x<<1,l,mid,tar);
	if(mid<tar) query2(x<<1|1,mid+1,r,tar);
	t[x]=t[x<<1]+t[x<<1|1];
}
int main()
{
